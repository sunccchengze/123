"""Reproducibly regenerate Paper 1 Figures 1--6.

Numerical values that already have an audited experiment cache are read from
``expcache/`` rather than copied into plotting code.  Only the Fig. 1 phase map
and the wind-direction-300-degree matrix used in Fig. 2 are evaluated when
missing; their raw values are then cached as JSON as well.

Environment: Python 3.11, FLORIS 4.6.6, NumPy, Matplotlib.
"""
