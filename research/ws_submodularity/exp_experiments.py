"""ARCHIVED P1/P2 exploratory analysis, not a validation script.

Its finite-difference, decoupling, DJS, certificate, and timing outputs were
used in claims withdrawn by the 2026-08-31 forensic audit. The routine named
``djs`` mutates coordinates in place and is cyclic Gauss--Seidel, not Jacobi.
Retained for traceability only; read ARCHIVE_NOTICE.md and run
p1_p2_forensic_audit.py for the corrective evidence.
"""
...
def djs(fm, y0, max_sweeps=6, grid_step=1.0, trace=False):
    """Historical in-place cyclic Gauss--Seidel sweep, misnamed DJS.

    Each later coordinate sees earlier edits in ``ynew``. This is not Jacobi,
    not parallel execution, and is retained solely to reproduce old outputs.
    """
