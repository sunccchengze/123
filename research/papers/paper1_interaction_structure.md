...
The drop is roughly an order of magnitude (7–70×) in every case with an interior optimum; the random weak-benefit case (gain +0.21 %) is the only one without a drop. Values are Frobenius norms, central differences h = 5°, optima from four-start SLSQP; the §9 sign-matrix study at DJS optima gives 0.020 (gauss) and 0.116 (cc) for the same farm, consistent within optimizer differences. The optimal profiles themselves are row-synchronous and monotonically decreasing downstream (the 4×4 optimum is rows at [30°, 25.5°, 16.5°, 0°]), consistent with the lattice structure of §3.
...
- Figures: fig1_phasemap (Fig. 1), fig2_signmatrices (Fig. 2), fig3_decoupling (Fig. 3), fig6_greedygaps (Fig. 4), fig4_ti_sweep (Fig. 5), fig5_y4x4 (Fig. 6), fig7_model_curves (Fig. 7), fig8_flip_models (Fig. 8), fig9_ws_decoupling (Fig. 9), fig10_windrose (Fig. 10). The DJS and quasi-concavity graphics are retained only in Papers 2 and 3, respectively, to avoid duplicate manuscript figures.
