...
Wake steering increases wind-farm power by yawing upstream turbines so that their wakes deflect away from downstream rows, and the field has accumulated a puzzle: sequential or "greedy" heuristics, which sweep turbines from upstream to downstream, recover 99.4–99.9 % of the gains of continuous optimization, yet no approximation guarantee exists, and the general yaw problem is strongly NP-hard. This paper resolves the puzzle by analyzing the second-order structure of the farm-power objective P(γ). We prove that every mixed partial ∂²P/∂γi∂γj decomposes into a *complementarity channel* (positive contributions from turbines lying downstream of both i and j) and a *substitution channel* (a negative contribution arising when j lies downstream of i, mediated by j's own yaw-dependent power factor cos^p(γj)). The sign of each turbine pair is therefore determined by the wake-interaction DAG and by the operating point: serial pairs are strategic substitutes, lateral pairs with shared downstream turbines are strategic complements, and chain-adjacent pairs undergo a complement→substitute phase flip that we map on FLORIS. Two consequences follow. First, at power-maximizing yaw profiles the Hessian of P is nearly diagonal: the off-diagonal-to-diagonal norm ratio drops from 0.27–0.97 at generic points to 0.022–0.068 at optima across 3×3, 4×4, misaligned, and AEP cases. Near the optimum the problem is therefore locally separable, which explains the success of serial-refine, Boolean-greedy, and coordinate-sweep methods and licenses parallel per-turbine optimization. Second, bounded interactions yield an explicit greedy gap bound; numerically, upstream-to-downstream greedy achieves a mean optimality gap of 0.103 % (max 0.477 %) over 12 random layouts (N = 6–9). The interaction sign matrix also acts as a flow-topology diagnostic, and monotone comparative statics (Topkis-type) explain why optimal yaw profiles decrease downstream and shrink with turbulence intensity, providing the missing mechanism for trends reported in recent sensitivity studies.
...
| 4×4, wd 270° | **0.055** | 0.521 | — |
| 3×3, AEP over 12 directions | **0.068** | 0.966 | — |
...
12. `optimal yaw decreases turbulence intensity` → Wynn 2023, King 2021, Quick 2020 empirical trends; our §7 supplies the mechanism. Cited, not claimed.
...
6. Bestehorn, F., et al.: Integer programming for optimal yaw control of wind farms, Wind Energ. Sci. 10, 1637, 2025. doi:10.5194/wes-10-1637-2025
...
9. Wynn, A., et al.: Sensitivity analysis of wake steering optimisation for wind farm power maximisation, Wind Energ. Sci. 8, 1425, 2023. doi:10.5194/wes-8-1425-2023
...
13. Laizet, S., et al.: Sensitivity of wind farm wake steering strategies to analytical wake models, TORQUE 2022 proceedings, 2022. doi:10.1201/9781003360773-75
...
21. Larsen, G. C., et al.: Integrated wind farm layout and control optimization, Wind Energ. Sci. 5, 1551–1567, 2020. doi:10.5194/wes-5-1551-2020
