...
\begin{abstract}
Real wind-farm control needs the inverse of the wake-steering map: given a power target, find the yaw profile that delivers it. The standard forward map from yaw profiles to farm power is neither injective nor convex, so the field usually avoids the inverse problem entirely; the state of the art stays in the forward world, with lookup tables, PI tracking, or re-optimization. This paper shows that the second-order structure of the companion paper makes the inverse problem tractable and certifiable. Farm power along any fixed ray is unimodal in the scale parameter, so tracking reduces to scalar bisection; the number of model calls is logarithmic in precision, compared with the linear cost of exhaustive search. Under the bounded-interaction condition, the power map is $K$-monotone with explicit $K$, which yields the exact inverse-monotonicity statement: a difference of $K$ between two power targets translates into a difference of at most $\gbar\sqrt{K}$ between the corresponding optimal scales, with an order-optimal bisection algorithm. We instantiate the tracking scheme on a $3\times3$ FLORIS farm: bisection reaches nine targets spanning the attainable range 8192--10022\,kW in 7--11 model evaluations each with error $\le 7.8\cdot10^{-4}$\,kW ($\approx8\cdot10^{-8}$ of $P_{\max}$); the bilinear-proxy reverse-search baseline of our companion engineering platform errs by up to 60.28\,kW (0.60\,\% of $P_{\max}$) at five evaluations, five orders of magnitude worse; and the one-sweep monotonicity pre-check (41 model calls) decides attainability of any requested target before serving it. The result is a new operating mode for wind farms: power scheduling with a well-posedness certificate, not a control loop.
\end{abstract}
...
\textbf{Contributions.} (i) The first structural analysis of the wake-steering tracking inverse: monotone rays, quasi-concave single-peak responses, and an attainable-range theorem (Sections~2--3); (ii) an exact, gradient-free inversion algorithm with measured error $\approx4\cdot10^{-8}$ of $P_{\max}$ (Section~4); (iii) a quantified upgrade path for interpolated-proxy trackers, five orders of magnitude at a comparable budget (Section~5); (iv) a per-wind-condition well-posedness certificate that runs in one 41-model-call sweep. To our knowledge this is the first power-tracking yaw scheme with an inverse-map guarantee.
...
Our companion platform ships a browser-side tracker: a bilinear interpolant of the ray response built from five grid points, inverted by reverse search. Measured maximum tracking error on the $3\times3$ power range: \textbf{60.28\,kW $= 0.60$\,\% of $P_{\max}$} (Fig.~\ref{fig:proxy}): the proxy's reverse search overestimates the attainable range near the peak and misses the overshoot branch of Section~3. Ray-bisection at a comparable budget (7--11 model calls versus the proxy's five grid points) achieves a maximum error of $7.8\cdot10^{-4}$\,kW, five orders of magnitude better, and the 41-sample pre-check tells the page whether the requested target is attainable at all, which the proxy cannot. Recommendation: keep the bilinear proxy for visualization; compute actual setpoints with ray-bisection (server- or WASM-side; both fit the existing evaluation loop); retire the reverse search.
...
\caption{Proxy versus exact inverse on the $3\times3$ power range: the bilinear-proxy reverse search errs by up to 60.28\,kW (0.60\,\% of $P_{\max}$); ray-bisection at a comparable budget (7--11 calls) reaches $\le7.8\cdot10^{-4}$\,kW, five orders of magnitude better.}
\label{fig:proxy}
\end{figure}
