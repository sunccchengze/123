...
Fig. 1a shows the convergence traces: after the first sweep, DJS has captured ≥99 % of the SLSQP gain on the structured farms (99.4 / 99.7 / 99.6 %) and 89.4 % on the 16-turbine random layout; the second sweep closes all cases to ≥99.8 %. the second and third sweeps resolve the residual cross-talk to below 0.005 %. Fig. 1b shows the wall-time crossover: in serial brute-force form DJS is 2–3× slower than 4-start SLSQP at equal N (e.g., 89 s vs 36 s at N = 16), but every line search parallelizes across turbines, so the *critical-path* time of one sweep is a single 31-evaluation line search, and golden-section search cuts the serial evaluations sixfold — both effects flip the comparison without touching the solution quality. With golden-section search (≈5 evaluations per line search instead of 31) the sweep cost drops a further 6×; we kept the brute-force version in the benchmark to make the comparison unfavorable to DJS.
...
- Loads: yaw rate limits and fatigue constraints enter as per-turbine box constraints — the decoupled sweeps handle them without modification (they are per-coordinate).
...
[Paper 1; Fleming et al. 2022; Stanley et al. 2022; Bestehorn et al. 2025; Starke et al. 2024; Park & Law 2015; King et al. 2021; Liu et al. 2020 (WGWD, Energies 13(4):865, doi:10.3390/en13040865); Richtárik & Takáč 2016 (parallel coordinate descent, Math. Program.); Wright 2015 (coordinate descent review, Math. Program.).]
