...
`figs/`：7 张图（PDF 矢量 + PNG 预览）。fig1 触发波（含幅值无关性双联）、fig2 相图、fig3 功率台阶（双尾流核）、fig4 **除颤阴性结果**三联、fig5 **验证路径**（短程幻觉 + 沉降时间=(N-1)L/U 验证）、fig6 五种子统计、fig7 τ_r×k 参数依赖热图、fig8 **消融实验**（阈值=台阶之源）。
